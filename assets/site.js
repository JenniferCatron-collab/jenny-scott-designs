...
(function(){
  ...
  function openLoginModal() {
    if (loginModal) {
      ...
      var err = document.getElementById('loginError'); if (err) err.innerHTML = '';
    }
  }
  ...
  function submitLogin() {
    var password = loginInput ? loginInput.value : '';
    ...
    fetch('/login', {
      ...
      if (j && j.authenticated) {
        ...
        if (errorDiv) errorDiv.innerHTML = '<p style="color:green;">Login successful!</p>';
        setTimeout(function() { closeLoginModal(); }, 800);
      } else {
        if (errorDiv) errorDiv.innerHTML = '<p style="color:red;">Invalid password</p>';
      }
    }).catch(function(err) {
      console.error('Login error:', err);
      if (errorDiv) errorDiv.innerHTML = '<p style="color:red;">Login error - please try again</p>';
    });
  }
    ...
    document.querySelectorAll('.image-description').forEach(function(desc){
      ...
      editBtn.addEventListener('click', function(e){
        ...
        saveBtn.addEventListener('click', function(e){
          ...
          fetch('/admin/save-snippet',{method:'POST',body:JSON.stringify({path:location.pathname.replace(/^\//,'')||'index.html',id:id,content:textarea.value}),headers:{'Content-Type':'application/json'}}).then(r=>r.json()).then(function(j){
            if(j && j.ok){
